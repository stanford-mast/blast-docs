# Concurrency in BLAST

<Note>
**Quick Guide to Controlling Concurrency**

For most users, you only need to know about two settings:
...
If you're interested in how BLAST's concurrency works under the hood or need more advanced features, continue reading this guide.
</Note>
...
BLAST supports several types of task relationships:
...
results = await engine.run([
    "Go to python.org",  # Parent task
    "Click on Documentation",  # Subtask 1
    "Search for 'asyncio'"  # Subtask 2
])
```
...
BLAST can execute multiple tasks in parallel when resources allow:
...
1. **Use Prerequisites** when tasks must run in sequence:
   ```python
   task1 = await engine.run("Go to website")
   task2 = await engine.run("Click login", previous_response_id=task1.id)
   ```

2. **Use Subtasks** for parallel operations:
   ```python
   results = await engine.run([
       "Open main page",
       "Check notifications",  # Runs in parallel
       "Check messages"       # Runs in parallel
   ])
   ```

3. **Monitor Resources** to avoid overload:
   ```python
   metrics = await engine.get_metrics()
   if metrics['memory_usage_gb'] > 7.0:  # High memory usage
       # Wait before scheduling more tasks
...
- Learn about [Parallelism](/guides/parallelism) for advanced concurrent execution
